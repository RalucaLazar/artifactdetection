...
public class KnnManager {
    ...
    public static int[] predict(String trainingFile, TestRecord[] testRecords, int k, Metric metric) {
        ...
        try {
            ...
            for (int j = 0; j < lengthOfTestingRecords; j++) {
                if (testRecords[j].getPredictedLabel() == testRecords[j].getClassLabel())
                    correctPrediction++;
            }
        } catch (IOException e) {
            e.printStackTrace();
     ...
     * neghbors.
     *
     * @param neighbors : set of points that was found to be closest neigbors
